...
def dweg (csp,var,assignment):
    sum=0
    for n in csp.neighbors[str(var)]:
        if n not in assignment:
            w+=csp.weight[(str(var),n)]
    return sum
...
def mrv(assignment, csp):
    ...
    return argmin_random_tie([v for v in csp.variables if v not in assignment],
                             key=lambda var: num_legal_values(csp, var, assignment)/dweg(csp,var,assignment))
